...
| `strip_cloudapi_prefix` | Optional boolean, defaults to `false`. When `true`, strips a leading `/cloudapi` path segment from every outgoing compute-API request. The SDK always targets `.../cloudapi/v6`, matching the public `api.ionos.com` surface and most mTLS-enforcing endpoints too - but some internal endpoints serve the same API directly at `/v6/...` with no `/cloudapi` segment. Requires `client_cert`/`client_key` to also be set - setting `strip_cloudapi_prefix` on its own is rejected as an error, same as `ca_cert`. Deliberately not usable independently of mTLS: it exists to work around an internal mTLS proxy quirk, and a plain (non-mTLS) endpoint using the standard `/cloudapi/v6` layout must never have its paths rewritten. |
