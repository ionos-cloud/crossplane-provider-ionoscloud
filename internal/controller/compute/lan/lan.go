...
func Setup(mgr ctrl.Manager, opts *utils.ConfigurationOptions) error {
	...
	if opts.CtrlOpts.MetricOptions != nil && opts.CtrlOpts.MetricOptions.MRStateMetrics != nil {
		stateMetricsRecorder := statemetrics.NewMRStateRecorder(
			mgr.GetClient(), opts.CtrlOpts.Logger, opts.CtrlOpts.MetricOptions.MRStateMetrics, &v1alpha1.LanList{}, opts.CtrlOpts.MetricOptions.PollStateMetricInterval,
		)
	...
	return ctrl.NewControllerManagedBy(mgr).
		...
		WithOptions(controller.Options{
			MaxConcurrentReconciles: opts.GetMaxConcurrentReconcileRate(v1alpha1.LanKind),
...
func (c *externalLan) Observe(ctx context.Context, mg resource.Managed) (managed.ExternalObservation, error) {
	...
	isUpToDate, diff := lan.IsUpToDate(cr, instance)
...
func (c *externalLan) Create(ctx context.Context, mg resource.Managed) (managed.ExternalCreation, error) { // nolint:gocyclo
	...
	if err != nil {
		return creation, fmt.Errorf("failed to extract request ID: %w", err)
	}
