...
func Setup(mgr ctrl.Manager, opts *utils.ConfigurationOptions) error {
	...
	if opts.CtrlOpts.MetricOptions != nil && opts.CtrlOpts.MetricOptions.MRStateMetrics != nil {
		stateMetricsRecorder := statemetrics.NewMRStateRecorder(
			mgr.GetClient(), opts.CtrlOpts.Logger, opts.CtrlOpts.MetricOptions.MRStateMetrics, &v1alpha1.ServerList{}, opts.CtrlOpts.MetricOptions.PollStateMetricInterval,
		)
	...
	return ctrl.NewControllerManagedBy(mgr).
		...
		WithOptions(controller.Options{
			MaxConcurrentReconciles: opts.GetMaxConcurrentReconcileRate(v1alpha1.ServerKind),
...
func (c *externalServer) Observe(ctx context.Context, mg resource.Managed) (managed.ExternalObservation, error) {
	...
	return managed.ExternalObservation{
		ResourceExists:          true,
		ResourceUpToDate:        server.IsUpToDate(cr, observed),
		ConnectionDetails:       managed.ConnectionDetails{},
		ResourceLateInitialized: !cmp.Equal(current, &cr.Spec.ForProvider),
		Diff:                    server.NeedsUpdate(cr, observed),
	}, nil
}
