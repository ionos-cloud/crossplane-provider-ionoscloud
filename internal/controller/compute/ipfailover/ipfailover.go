...
func Setup(mgr ctrl.Manager, opts *utils.ConfigurationOptions) error {
	...
	if opts.CtrlOpts.MetricOptions != nil && opts.CtrlOpts.MetricOptions.MRStateMetrics != nil {
		stateMetricsRecorder := statemetrics.NewMRStateRecorder(
			mgr.GetClient(), opts.CtrlOpts.Logger, opts.CtrlOpts.MetricOptions.MRStateMetrics, &v1alpha1.IPFailoverList{}, opts.CtrlOpts.MetricOptions.PollStateMetricInterval,
		)
	...
	return ctrl.NewControllerManagedBy(mgr).
		...
		WithOptions(controller.Options{
			MaxConcurrentReconciles: opts.GetMaxConcurrentReconcileRate(v1alpha1.IPFailoverKind),
