...
func Setup(mgr ctrl.Manager, opts *utils.ConfigurationOptions) error {
	...
	if opts.CtrlOpts.MetricOptions != nil && opts.CtrlOpts.MetricOptions.MRStateMetrics != nil {
		stateMetricsRecorder := statemetrics.NewMRStateRecorder(
			mgr.GetClient(), opts.CtrlOpts.Logger, opts.CtrlOpts.MetricOptions.MRStateMetrics, &v1alpha1.DataplatformNodepoolList{}, opts.CtrlOpts.MetricOptions.PollStateMetricInterval,
		)
	...
	return ctrl.NewControllerManagedBy(mgr).
		...
		WithOptions(controller.Options{
			MaxConcurrentReconciles: opts.GetMaxConcurrentReconcileRate(v1alpha1.DataplatformNodepoolKind),
...
func (c *externalDataplatform) Observe(ctx context.Context, mg resource.Managed) (managed.ExternalObservation, error) {
	...
	c.log.Debug("Observed dp nodepool: ", "state", cr.Status.AtProvider.State, "external name", meta.GetExternalName(cr), "name", cr.Spec.ForProvider.Name)
