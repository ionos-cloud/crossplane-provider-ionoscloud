...
func (c *externalCluster) Observe(ctx context.Context, mg resource.Managed) (managed.ExternalObservation, error) { // nolint:gocyclo
	...
	newCtx, cancel := context.WithTimeout(ctx, 30*time.Minute)
	defer cancel()
	observed, resp, err := c.service.GetK8sCluster(newCtx, meta.GetExternalName(cr))
	if err != nil {
	...
	case string(ionoscloud.AVAILABLE), k8s.ACTIVE:
		c.log.Debug("set available")
		cr.SetConditions(xpv1.Available())
	case string(ionoscloud.DESTROYING):
		c.log.Debug("set deleting")
		cr.SetConditions(xpv1.Deleting())
	case string(ionoscloud.BUSY), k8s.DEPLOYING:
		c.log.Debug("set creating")
		cr.SetConditions(xpv1.Creating())
	default:
		c.log.Debug("set unavailable")
...
func (c *externalCluster) Update(ctx context.Context, mg resource.Managed) (managed.ExternalUpdate, error) {
	...
	_, apiResponse, err := c.service.UpdateK8sCluster(ctx, clusterID, *instanceInput)
	if err != nil {
...
func (c *externalCluster) Delete(ctx context.Context, mg resource.Managed) error {
	...
	_, err := c.service.DeleteK8sCluster(ctx, cr.Status.AtProvider.ClusterID)
